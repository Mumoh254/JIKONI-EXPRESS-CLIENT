...
import {
    ...
    FiUser,
    FiSmartphone,
    FiUsers, // Changed from FiUserCheck to FiUsers for a more general 'role' icon
    FiLoader
} from 'react-icons/fi';
...
// --- Jikoni Express Color Palette (Retained from previous context) ---
const colors = {
...
// --- Styled Components (Updated for consistency and role field) ---
...
const AuthContainer = styled.div`
    max-width: 480px; /* Slightly wider to accommodate new field if needed */
    width: 100%;
    padding: 2.5rem;
...
const IconWrapper = styled.div`
    ...
    color: ${colors.placeholderText};
    font-size: 1.2rem;
    pointer-events: none; /* Ensure icon doesn't block clicks on select */
`;
...
// --- Validation Schema (Updated to include Role) ---
const validationSchema = Yup.object().shape({
    ...
    Role: Yup.string() // New Role validation
        .required('Role is required')
        .oneOf(['ADMIN', 'MANAGER'], 'Invalid role selection'),
    Password: Yup.string()
...
const AdminRegister = () => { // Renamed component for clarity
    ...
    const handleSubmit = async (values, { setSubmitting, resetForm }) => {
        try {
            ...
            if (response.status === 201 || response.status === 200) {
                // Assuming your backend sends a token or similar identifier upon successful admin registration
                // localStorage.setItem('adminToken', response.data.token); // Consider different token key for admins
                setRegistrationSuccess(true);
                resetForm();
                setTimeout(() => {
                    navigate('/admin-login'); // Redirect to an admin-specific login page
                }, 2000);
            } else {
                setServerError(response.data?.message || 'Admin registration failed with an unexpected status.');
            }
        } catch (error) {
            setServerError(error.response?.data?.message || 'Admin registration failed. Please try again.');
        } finally {
            setSubmitting(false);
        }
    };
        ...
        <PageWrapper>
            <AuthContainer>
                <Header>
                    ...
                    <h1>Jikoni Express Admin Registration</h1>
                    <p>Create an admin or manager account for your team.</p>
                </Header>
                ...
                >
                    {({ isSubmitting }) => (
                        <Form>
                            ...
                            {/* Gender Field */}
                            <FormGroup>
                                <IconWrapper><FiUser /></IconWrapper> {/* Using FiUser for gender */}
                            ...
                            {registrationSuccess && (
                                <SuccessText>
                                    <FiCheckCircle /> Successfully registered! Redirecting to admin login...
                                </SuccessText>
                            )}
                            ...
                            <LinkText>
                                ...
                                <Link to="/admin-login"> {/* Link to an admin-specific login */}
                                    Login here
                                </Link>
                            </LinkText>
                        </Form>
...
export default AdminRegister; // Export the renamed component
